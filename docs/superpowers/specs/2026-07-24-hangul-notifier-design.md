...
- [ ] 메모장/Word/Chrome 주소창·텍스트영역/VS Code/카카오톡에서 `되요` 감지.
- [ ] 알림 중 IME 조합 미중단(`한글되요`).
- [ ] 정상 문장 오탐 0. [ ] 비번란 알림 없음.
- [ ] 8시간 상주 후 메모리 +20MB 미만, 후킹 생존. [ ] 콜백 평균 1ms 미만.
- [ ] `HangulAutomata` 커버리지 90%+. [ ] 일시정지→재개 정상. [ ] 로그에 입력 텍스트 0글자.
